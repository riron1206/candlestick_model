...
def _valid_plot_kwargs():
    ...
    vkwargs = {
        ...
        "style": {
            "Default": "default",
            "Validator": lambda value: value in _styles.available_styles()
        ...
        "mav": {"Default": None, "Validator": _mav_validator},
        "renko_params": {
            "Default": dict(),
        ...
        "figscale": {
            "Default": 1.0,  # scale base figure size up or down.
        ...
        "addplot": {
            "Default": None,
            "Validator": lambda value: isinstance(value, dict)
            or (isinstance(value, list) and all([isinstance(d, dict) for d in value])),
        },
        "savefig": {
            "Default": None,
            "Validator": lambda value: isinstance(value, dict)
        ...
        "block": {"Default": True, "Validator": lambda value: isinstance(value, bool)},
        "returnfig": {
            "Default": False,
            "Validator": lambda value: isinstance(value, bool),
        },
...
def plot(data, is_not_set_visible=False, **kwargs):
    ...
    if addplot is not None:
        if isinstance(addplot, dict):
            ...
            raise TypeError(
                "addplot must be `dict`, or `list of dict`, NOT " + str(type(addplot))
            )
    ...
    if avg_days_between_points < 0.33:  # intraday
        ...
        if (
            mdates.num2date(dates[-1]).date().year
            != mdates.num2date(dates[0]).date().year
        ):
            fmtstring = "%Y-%b-%d"
        else:
            fmtstring = "%b %d"
    ...
    if ptype == "candle" or ptype == "candlestick":
        collections = _construct_candlestick_collections(
            xdates, opens, highs, lows, closes, marketcolors=style["marketcolors"]
        )
    ...
    if config["volume"]:
        ...
        vcolors = _updown_colors(
            vup, vdown, opens, closes, use_prev_close=style["marketcolors"]["vcdopcod"]
        )
    ...
    if config["savefig"] is not None:
        ...
        if isinstance(save, dict):
            ...
            plt.savefig(save, bbox_inches="tight", pad_inches=0)  # 余白削除オプション付けた
            # plt.savefig(save)
